/*
 ...
 DECLARATION OF IMPLICIT OBJECTS ACCROSS THE FRAMEWORK
...
(function(window) {
	...
	//TODO: Make non-static so we can have several Pinta objects at a time.
	...
	var Pinta = {
		...
		// Initializes the pinta object with the passed root parameter as the base DOM element to make selections on
		init: function(root, conf, params) {

			Log.info("Initializing Pinta object with root " + root);

			// Checking for root
			if (typeof root !== "object") {
				Log.error("No root passed when initializing Pinta");
				return;
			}
			this.root = root;
			...
			if (typeof conf == "string") {
				...
				Log.error("No conf passed when initializing Pinta");
				return;
			}
			...
			Log.info("New Pinta session created with id: " + this.session.id);

		},

		// Unsets the pinta object cleaning up configuration and references
		destroy: function() {
			Log.debug("Destroying Pinta object");
			this.session = {};
			this.root = this._conf = this._params = this._currentPage = this._displayedPage = null;
		},

		// Returns whether the Pinta object is initialized
		isInit: function() {
		...
		goto: function(id, params) {
			...
			if (!this.isInit()) {
				Log.warn("Cannot goto " + id + " - Pinta is not yet initalized");
				return;
			}
		...
		getPageContext: function(id) {
			...
			if (!this.isInit()) {
				Log.warn("Cannot getPageContext (" + id + ")- Pinta is not yet initalized");
				return;
			}
		...
		_createSessionId: function() {
			if (window._nPinta == null)
				window._nPinta = 0;
			window._nPinta++;
			return _NAME + window._nPinta;
		}
	...
	// Expose pinta to the global object and init
	window.Pinta = Pinta;
